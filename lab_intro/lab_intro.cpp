...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {
	...
	double NewLumin = sqrt((deltaY*deltaY) + (deltaX*deltaX));
	if(NewLumin >= 200){NewLumin=200;}
	NewLumin = 1-0.005*NewLumin;
      HSLAPixel *pixel = image.getPixel(x, y);
      pixel->l = pixel->l*NewLumin;
