...
class KDTree
{
  ...
  private:
     ...
     */
	void QuickSelect(int first, int last, int k, int CurDim);
	int Partition(int first, int last, int mid_index, int CurDim);
	Point<Dim> NearestNeighborHelper(int CurDim, const Point<Dim> &query, int first, int last, const Point<Dim> &CurBest) const;
};
