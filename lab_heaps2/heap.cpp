...
size_t heap<T, Compare>::maxPriorityChild(size_t currentIdx) const
{
	...
	if(2*currentIdx+1 < _elems.size() ){
		if( higherPriority( _elems[this->leftChild(currentIdx)], _elems[this->rightChild(currentIdx)] ) ){
			return this->leftChild(currentIdx);
		}else{
			return this->rightChild(currentIdx);
		}
	}
    return 2*currentIdx;
}
