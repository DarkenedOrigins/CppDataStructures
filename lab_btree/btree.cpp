...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    *
	...
	new_right->elements.insert(new_right->elements.end(), mid_elem_itr+1, child->elements.end());
	new_right->children.insert(new_right->children.end(), mid_child_itr+1, child->children.end());
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	if(subroot->is_leaf){
		subroot->elements.insert(subroot->elements.begin()+first_larger_idx-1,pair);
		return;
	}
