...
void TileImage::paste(PNG& canvas, int startX, int startY, int resolution)  {
	...
	if( (int)image_.width() == resolution && (int)image_.height() == resolution){
		for(int i=0; i<resolution; i++){
			for(int j=0; j<resolution; j++){
				*(canvas.getPixel(startX+i, startY+j) ) = *(image_.getPixel(i , j) );
			}
		}
	}
	else if (image_.width() % resolution == 0) {
        int scalingRatio = image_.width() / resolution;
		PNG newImg(resolution, resolution);
		HSLAPixel NewPix;
        for (int x = 0; x < resolution; x++) {
            for (int y = 0; y < resolution; y++) {
                ...
                NewPix = getScaledPixelInt(pixelStartX, pixelEndX, pixelStartY, pixelEndY);
				*(canvas.getPixel(startX + x, startY + y)) = NewPix;
				*(newImg.getPixel(x,y) ) = NewPix;
            }
        }
		image_ = newImg;
    } else { // scaling is necessary
        double scalingRatio = static_cast<double>(image_.width()) / resolution;
		PNG newImg(resolution, resolution);
		HSLAPixel NewPix;
        ...
        for (int x = 0; x < resolution; x++) {
            for (int y = 0; y < resolution; y++) {
                ...
                NewPix = getScaledPixelDouble(pixelStartX, pixelEndX, pixelStartY, pixelEndY);
				*(canvas.getPixel(startX + x, startY + y)) = NewPix;
				*(newImg.getPixel(x,y) ) = NewPix;
            }
        }
