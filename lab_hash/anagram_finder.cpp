...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
     ...
     */
	Dict<char, int> WordHash(11); //avg length of words in english is 5 so i did 11 to be safe
	Dict<char, int> TestHash(11); 
	for(auto WordIt = word.begin(); WordIt != word.end(); WordIt++){
		WordHash[*WordIt]++;
	}
	for(auto TestIt = test.begin(); TestIt != test.end(); TestIt++){
		TestHash[*TestIt]++;
	}
	for(auto WordHashIt = WordHash.begin(); WordHashIt != WordHash.end(); WordHashIt++){
		if( TestHash[WordHashIt->first] != WordHashIt->second ){
			return false;
		}
	}
	return true;
